...
var MovementHelper = /** @class */ (function () {
    ...
    MovementHelper.prototype.doTeleport = function (player, _a) {
        ...
        var realTile = this.getTileAt(player.map, player.x, player.y);
    ...
    MovementHelper.prototype.handleTile = function (player, tile, ignoreIf) {
        ...
        if (forceEvent) {
            ...
            if (forceEvent !== Event_1.EventName.Providence) {
    ...
    MovementHelper.prototype.handleTileTrainer = function (player, tileData) {
        ...
        this.eventManager.doEventFor(player, Event_1.EventName.FindTrainer, { professionName: professionName, trainerName: trainerName });
    };
    MovementHelper.prototype.handleTileBoss = function (player, tileData) {
        this.eventManager.doEventFor(player, Event_1.EventName.BattleBoss, { bossName: tileData.object.name });
    };
    MovementHelper.prototype.handleTileBossParty = function (player, tileData) {
        this.eventManager.doEventFor(player, Event_1.EventName.BattleBoss, { bossParty: tileData.object.name });
    };
    MovementHelper.prototype.handleTileTreasure = function (player, tileData) {
        this.eventManager.doEventFor(player, Event_1.EventName.FindTreasure, { treasureName: tileData.object.name });
    };
    MovementHelper.prototype.takeStep = function (player) {
        ...
        var oldLoc = { x: player.x, y: player.y };
        if (player.stepCooldown > 0) {
            player.stepCooldown--;
        }
        player.stepCooldown--;
